...
func (c createDraftAction) Perform(ctx context.Context, update telegram.Update) error {
	...
	_, err = member.CreateDraft(ctx, title, description)
	if err != nil {
		return fmt.Errorf("creating draft task in team %d: %w", teamID, err)
	}
	return nil
}

func CreateDraft(community domain.Community, forms forms.Forms) core.Action {
