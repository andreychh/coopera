...
* [3. Исключенные возможности и неподдерживаемые сценарии](#3-исключенные-возможности-и-неподдерживаемые-сценарии)
    * [3.1. Исключенные пользовательские возможности](#31-исключенные-пользовательские-возможности)
    * [3.2. Исключенные функциональные и технические возможности](#32-исключенные-функциональные-и-технические-возможности)
    * [3.3. Исключенные аппаратные и программные требования](#33-исключенные-аппаратные-и-программные-требования)
...
В данном разделе приведены функциональные, пользовательские, аппаратные и программные требования, реализация которых  
**не предусмотрена в текущем релизе проекта Coopera (осенний семестр 2025 года)**. Для каждого исключенного элемента указаны обоснования  
и возможные перспективы включения в будущие версии.

### 3.1. Исключенные пользовательские возможности

| Возможность | Обоснование | Перспектива реализации |
|------------|------------|----------------------|
| Мобильное приложение (Android/iOS) | Требует отдельной команды и значительных временных затрат. На текущем этапе акцент на веб-интерфейсе и Telegram-боте. | Возможно в следующем семестре на основе API текущей версии. |
| Продвинутая система уведомлений (email, push) | Требует внешней интеграции и серверов для доставки уведомлений. В текущем релизе уведомления ограничены Telegram-ботом. | Планируется к реализации при выходе проекта в стадию публичного тестирования. |
| Механизм обратной связи через интерфейс | Второстепенная функциональность, не влияющая на основные бизнес-процессы. | Возможна после MVP для повышения вовлеченности пользователей. |
| Поддержка многоязычного интерфейса | Система предназначена для внутреннего использования русскоязычными пользователями. | Будет рассмотрено при масштабировании проекта. |

### 3.2. Исключенные функциональные и технические возможности

| Возможность | Обоснование | Перспектива реализации |
|------------|------------|----------------------|
| Интеграция с внешними таск-трекерами (Trello, Jira) | Требует дополнительной проработки API и OAuth-авторизации. | Возможно в следующем релизе для корпоративных пользователей. |
| Расширенные права доступа и ролевые модели (admin, manager, viewer) | На данном этапе используется упрощенная ролевая модель для минимизации сложности реализации. | Планируется после стабилизации базового функционала. |
| Автоматическое распределение задач на основе загрузки участников | Алгоритмы требуют накопления данных и тестирования. | Возможно при переходе к версии 2.0 после анализа пользовательской активности. |
| Система аналитики и отчетов | Не входит в минимальный функционал текущего этапа, требует отдельного хранилища и визуализаций. | Рассматривается в будущем при внедрении бизнес-аналитики. |

### 3.3. Исключенные аппаратные и программные требования

| Возможность | Обоснование | Перспектива реализации |
|------------|------------|----------------------|
| Поддержка Windows Server и macOS как серверных ОС | Основное окружение — Linux (Ubuntu). Расширение списка ОС потребует дополнительных тестов и адаптаций Docker-конфигураций. | Возможно при переходе на кросс-платформенные решения. |
| Масштабирование в облачных кластерах (Kubernetes) | Для учебного этапа достаточно развертывания через Docker Compose на одном сервере. | Рассматривается при переходе к промышленной эксплуатации. |
| Поддержка высокодоступных и отказоустойчивых кластеров БД | Текущий релиз рассчитан на единый сервер с PostgreSQL и Redis. Отказоустойчивость обеспечивается стандартными средствами. | Планируется для корпоративного релиза с требованиями высокой нагрузки. |
