...
| **Тестирование: go test** | Стандартный инструмент для модульного тестирования. Интеграционные тесты также выполняются через `go test`, но в окружении с контейнерами (PostgreSQL, Redis) для проверки поведения системы в условиях, приближённых к продакшену.                                                                            |
