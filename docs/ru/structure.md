...
1. **Создание запроса**:
    - Автор изменения создаёт issue в GitHub с описанием изменения.
2. **Рассмотрение**:
...
3. **Реализация**:
    - После одобрения создаётся ветка. Имя ветки — номер соответствующего issue.
    - Изменения реализуются и тестируются локально.
4. **Запрос на внесение изменений**:
    - Создаётся PR с описанием изменений, со ссылкой на issue.
    - Обязательные проверки: автоматические тесты CI, линтеры, сборка артефактов.
5. Ревью:
    - Минимум один независимый разработчик выполняет код-ревью.
    - После прохождения ревью и успешного прохождения CI, PR сливается в `main`.
